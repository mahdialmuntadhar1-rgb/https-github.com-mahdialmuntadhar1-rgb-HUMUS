...
// Transform Supabase business row → BELIVE Business format
...
function transformBusiness(data: any): Business {
  ...
  return {
    ...
    imageUrl:
      ...
      `https://picsum.photos/seed/${data.id || "business"}/400/300`,
...
export async function getBusinessesFromSupabase(options?: {
  ...
  try {
    ...
    query = query.range(start, start + pageSize - 1).order("name", { ascending: true });
...
// Export mapping utilities for use in other services
export { DB_CATEGORY_TO_UI, UI_CATEGORY_TO_DB, transformBusiness };
